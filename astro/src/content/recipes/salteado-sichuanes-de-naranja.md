---
cooking_time: 30
created: '2026-01-14T19:23:08.447329+01:00'
id: 140
image: null
ingredients:
- id: 1814
  ingredient:
    name: Tofú
    prefix: ''
  optional: false
  quantity: '280'
  recipe: 140
  unit: gr
- id: 1815
  ingredient:
    name: maicena
    prefix: ''
  optional: false
  quantity: '2.5'
  recipe: 140
  unit: tablespoon
- id: 1816
  ingredient:
    name: cebolla marrón
    prefix: cortada en rodajas
  optional: false
  quantity: '1'
  recipe: 140
  unit: unit
- id: 1817
  ingredient:
    name: pimiento verde
    prefix: picado toscamente
  optional: false
  quantity: '1'
  recipe: 140
  unit: unit
- id: 1818
  ingredient:
    name: aceite
    prefix: neutro
  optional: false
  quantity: '2'
  recipe: 140
  unit: tablespoon
- id: 1819
  ingredient:
    name: granos de pimienta
    prefix: Sichuan
  optional: false
  quantity: '1'
  recipe: 140
  unit: tablespoon
- id: 1820
  ingredient:
    name: jengibre
    prefix: molido
  optional: false
  quantity: '2'
  recipe: 140
  unit: teaspoon
- id: 1821
  ingredient:
    name: ajo
    prefix: en polvo
  optional: false
  quantity: '2'
  recipe: 140
  unit: teaspoon
- id: 1822
  ingredient:
    name: zumo de naranja
    prefix: comercial
  optional: false
  quantity: '250'
  recipe: 140
  unit: ml
- id: 1823
  ingredient:
    name: salsa de soja
    prefix: ''
  optional: false
  quantity: '2'
  recipe: 140
  unit: tablespoon
- id: 1824
  ingredient:
    name: vinagre de arroz
    prefix: ''
  optional: false
  quantity: '2'
  recipe: 140
  unit: teaspoon
- id: 1825
  ingredient:
    name: azúcar
    prefix: ''
  optional: false
  quantity: '2'
  recipe: 140
  unit: teaspoon
- id: 1826
  ingredient:
    name: arroz
    prefix: cocido al vapor, para servir
  optional: false
  quantity: ''
  recipe: 140
  unit: unit
- id: 1827
  ingredient:
    name: naranja
    prefix: cortada en rodajas, para decorar
  optional: false
  quantity: '1'
  recipe: 140
  unit: unit
references:
- https://www.instagram.com/reel/DPmEiSQDGuv/
servings: 2
status: Draft
tags:
- cena
- vegetariano
- picante
title: Salteado Sichuanés de Naranja
updated: '2026-01-14T19:23:08.447379+01:00'
---
1. **Tostar los granos de pimienta Sichuan:** Tostar los granos de pimienta Sichuan en una sartén grande, caliente y seca hasta que estén fragantes. Retirarlos y molerlos hasta obtener un polvo fino con un mortero o una batidora. Reservar la sartén.
2. **Preparar el Tofú:** Cortar el tofú toscamente con unas tijeras (esto crea bordes ligeramente abrasivos que se crujen y retienen mejor la salsa). Mezclar con la mitad de la maicena, sal y pimienta. Añadir el aceite a la sartén reservada a fuego medio-alto. Incorporar el tofú y saltear hasta que esté dorado. Añadir la cebolla y el pimiento y cocinar durante 2-3 minutos más hasta que estén blandos pero aún crujientes.
3. **Añadir la salsa:** Preparar una mezcla de maicena batiendo la maicena restante con el zumo de naranja. Incorporar los granos de pimienta Sichuan molidos, la salsa de soja, el vinagre de arroz, el azúcar y la mezcla de naranja. Llevar a ebullición suave, removiendo durante un par de minutos hasta que espese. Probar y sazonar con sal si es necesario.
4. **Servir:** Servir con arroz y decorar con las rodajas de naranja.

*Información nutricional: Los valores nutricionales son aproximados y pueden variar según los ingredientes exactos utilizados.*
